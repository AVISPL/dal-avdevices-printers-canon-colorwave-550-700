...
public class ColourWave700 extends BaseDevice implements Monitorable {
    private final List<String> snmpOids = new ArrayList<String>(){{
        ...
        add(".1.3.6.1.2.1.1.3.0"); //DeviceUptime
    ...
    private final List<String> snmpNames = new ArrayList<String>(){{
        add("DeviceType");
        add("DeviceUptime");
        add("DevicePhysicalAddress");
        add("DeviceIpAddress");
        add("StatusDevice"); // INTEGER {unknown(1),running(2),warning(3),testing(4),down(5)}
        add("StatusDeviceErrorsCount"); //INT CountOfErrors
        add("StatusPrintState"); // INTEGER {other(1),unknown(2),idle(3),printing(4),warmup(5)}
        add("StatusPrinterErrors"); //Special case (BIT level errors)
    ...
    public List<Statistics> getMultipleStatistics() throws Exception {
        ...
        while (snmpIt.hasNext() && nameIt.hasNext()) {
            ...
            if (value.equals("noSuchObject") || value.equals("Request timed out") || value.equals("noSuchInstance")){
                continue;
            }
            ...
            switch(name){
                ...
                case "StatusPrinterErrors": addToMap(stats,name,getBitLevelErrors(value));break;
                case "InputName1"   : addToMap(stats,"Media "  + value,querySnmp(new ArrayList<String>(){{add(".1.3.6.1.2.1.43.8.2.1.12.1.1");}}).iterator().next().getValue()); break;
                case "InputName2"   : addToMap(stats,"Media "  + value,querySnmp(new ArrayList<String>(){{add(".1.3.6.1.2.1.43.8.2.1.12.1.2");}}).iterator().next().getValue()); break;
                case "InputName3"   : addToMap(stats,"Media "  + value,querySnmp(new ArrayList<String>(){{add(".1.3.6.1.2.1.43.8.2.1.12.1.3");}}).iterator().next().getValue()); break;
                case "InputName4"   : addToMap(stats,"Media "  + value,querySnmp(new ArrayList<String>(){{add(".1.3.6.1.2.1.43.8.2.1.12.1.4");}}).iterator().next().getValue()); break;
                case "InputName5"   : addToMap(stats,"Media "  + value,querySnmp(new ArrayList<String>(){{add(".1.3.6.1.2.1.43.8.2.1.12.1.5");}}).iterator().next().getValue()); break;
                case "InputName6"   : addToMap(stats,"Media "  + value,querySnmp(new ArrayList<String>(){{add(".1.3.6.1.2.1.43.8.2.1.12.1.6");}}).iterator().next().getValue()); break;
                case "CoverDesc1"   : addToMap(stats,"Status " + value,getCoverStatus(".1.3.6.1.2.1.43.6.1.1.3.1.1")); break;
                case "CoverDesc2"   : addToMap(stats,"Status " + value,getCoverStatus(".1.3.6.1.2.1.43.6.1.1.3.1.2")); break;
                case "CoverDesc3"   : addToMap(stats,"Status " + value,getCoverStatus(".1.3.6.1.2.1.43.6.1.1.3.1.3")); break;
                case "CoverDesc4"   : addToMap(stats,"Status " + value,getCoverStatus(".1.3.6.1.2.1.43.6.1.1.3.1.4")); break;
                case "CoverDesc5"   : addToMap(stats,"Status " + value,getCoverStatus(".1.3.6.1.2.1.43.6.1.1.3.1.5")); break;
                case "CoverDesc6"   : addToMap(stats,"Status " + value,getCoverStatus(".1.3.6.1.2.1.43.6.1.1.3.1.6")); break;
            }
        }
